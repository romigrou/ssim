...
RMGR_SSIM_BUILD_CLI    ?= ON
RMGR_SSIM_BUILD_SAMPLE ?= ON
RMGR_SSIM_BUILD_TESTS  ?= ON
RMGR_SSIM_USE_OPENMP   ?= ON
RMGR_SSIM_USE_DOUBLE   ?= OFF

GTEST_FILTER  ?= *
CMAKE_OPTIONS := "-DRMGR_SSIM_BUILD_CLI=$(RMGR_SSIM_BUILD_CLI)" "-DRMGR_SSIM_BUILD_SAMPLE=$(RMGR_SSIM_BUILD_SAMPLE)" "-DRMGR_SSIM_BUILD_TESTS=$(RMGR_SSIM_BUILD_TESTS)" "-DRMGR_SSIM_USE_OPENMP=$(RMGR_SSIM_USE_OPENMP)" "-DRMGR_SSIM_USE_DOUBLE=$(RMGR_SSIM_USE_DOUBLE)"

SOURCE_DIR    := $(HERE)
CMAKE_TARGETS := rmgr-ssim-tests rmgr-ssim-sample rmgr-ssim install package all
...
ifdef DESTDIR
    BUILDER_OPTIONS += DESTDIR=$(abspath $(DESTDIR))
endif
...
CMAKE_OPTIONS += "-DCPACK_PACKAGE_DIRECTORY=$(CPACK_PACKAGE_DIRECTORY)"
ifdef CMAKE_INSTALL_PREFIX
    CMAKE_OPTIONS += "-DCMAKE_INSTALL_PREFIX=$(CMAKE_INSTALL_PREFIX)"
else ifdef prefix
